...
基於跨語言一致性與維護便利性，專案已開始統一 `C#` 與 `Java` 的資料夾結構，使兩者的命名空間／套件結構保持對齊。

### Refact
- 改善 `Picture Clause` 相關程式碼的閱讀性。
    - 將基礎元件集中至 `Base` 或 `Core` 資料夾中，統一結構與責任劃分。
    - 處理 `NumericMeta` 的相依關係，解決原先反向參照（circular dependency）的狀況，降低模組耦合度。
...
### Changed
- 支援 `.NET 10`。
- 目標框架從 `net8.0` 擴展至 `net8.0;net10.0`。
